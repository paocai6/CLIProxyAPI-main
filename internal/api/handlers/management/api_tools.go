...
func (h *Handler) refreshAntigravityOAuthAccessToken(ctx context.Context, auth *coreauth.Auth) (string, error) {
	...
	clientID, clientSecret, errCreds := antigravityAuth.OAuthClientCredentials()
	if errCreds != nil {
		return "", errCreds
	}
	...
	req, errReq := http.NewRequestWithContext(ctx, http.MethodPost, antigravityAuth.TokenEndpoint, strings.NewReader(form.Encode()))
	if errReq != nil {
		return "", errReq
	}
