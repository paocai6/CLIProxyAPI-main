...
func (h *Handler) RequestGeminiCLIToken(c *gin.Context) {
	...
	clientID, clientSecret, errCreds := geminiAuth.OAuthClientCredentials()
	if errCreds != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": errCreds.Error()})
		return
	}
...
func (h *Handler) RequestAntigravityToken(c *gin.Context) {
	...
	if strings.TrimSpace(antigravity.OAuthClientID()) == "" {
		c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("antigravity oauth client id not configured: set %s", antigravity.OAuthClientIDEnv)})
		return
	}
