...
export function getVisualConfigValidationErrors(
  ...
  return {
    ...
    'pacing.maxConcurrency': getNonNegativeIntegerError(values.pacing.maxConcurrency),
    'pacing.minIntervalMs': getNonNegativeIntegerError(values.pacing.minIntervalMs),
    'pacing.jitterMs': getNonNegativeIntegerError(values.pacing.jitterMs),
  };
}
...
function mergeVisualConfigValues(
  ...
  if (patch.pacing) {
    nextValues.pacing = { ...currentValues.pacing, ...patch.pacing };
  }
  return nextValues;
}
  ...
  if (patch.pacing) {
    ...
    if (Object.prototype.hasOwnProperty.call(pacingPatch, 'maxConcurrency')) {
      updateDirty(
        ...
        nextValues.pacing.maxConcurrency === baselineValues.pacing.maxConcurrency
      );
    }
    if (Object.prototype.hasOwnProperty.call(pacingPatch, 'minIntervalMs')) {
      updateDirty(
        'pacing.minIntervalMs',
        nextValues.pacing.minIntervalMs === baselineValues.pacing.minIntervalMs
      );
    }
    if (Object.prototype.hasOwnProperty.call(pacingPatch, 'jitterMs')) {
      updateDirty(
        'pacing.jitterMs',
        nextValues.pacing.jitterMs === baselineValues.pacing.jitterMs
      );
    }
  }
...
export function useVisualConfig() {
  ...
  const loadVisualValuesFromYaml = useCallback((yamlContent: string) => {
    try {
      ...
      const pacing = asRecord(parsed.pacing);
      ...
      const newValues: VisualConfigValues = {
        ...
        pacing: {
          enable: Boolean(pacing?.enable),
          maxConcurrency: String(pacing?.['max-concurrency'] ?? '1'),
          minIntervalMs: String(pacing?.['min-interval-ms'] ?? '2000'),
          jitterMs: String(pacing?.['jitter-ms'] ?? '3000'),
        },
      };
    ...
    (currentYaml: string): string => {
      try {
        ...
        if (
          ...
          values.pacing.maxConcurrency.trim() ||
          values.pacing.minIntervalMs.trim() ||
          values.pacing.jitterMs.trim()
        ) {
          ensureMapInDoc(doc, ['pacing']);
          setBooleanInDoc(doc, ['pacing', 'enable'], values.pacing.enable);
          setIntFromStringInDoc(doc, ['pacing', 'max-concurrency'], values.pacing.maxConcurrency);
          setIntFromStringInDoc(doc, ['pacing', 'min-interval-ms'], values.pacing.minIntervalMs);
          setIntFromStringInDoc(doc, ['pacing', 'jitter-ms'], values.pacing.jitterMs);
          deleteIfMapEmpty(doc, ['pacing']);
        }
