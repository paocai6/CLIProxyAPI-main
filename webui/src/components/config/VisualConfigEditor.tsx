...
export function VisualConfigEditor({
  ...
  const maxConcurrencyInputId = useId();
  const maxConcurrencyHintId = `${maxConcurrencyInputId}-hint`;
  const maxConcurrencyErrorId = `${maxConcurrencyInputId}-error`;
  const minIntervalMsInputId = useId();
  const minIntervalMsHintId = `${minIntervalMsInputId}-hint`;
  const minIntervalMsErrorId = `${minIntervalMsInputId}-error`;
  const jitterMsInputId = useId();
  const jitterMsHintId = `${jitterMsInputId}-hint`;
  const jitterMsErrorId = `${jitterMsInputId}-error`;
  ...
  const maxConcurrencyError = getValidationMessage(
    t,
    validationErrors?.['pacing.maxConcurrency']
  );
  const minIntervalMsError = getValidationMessage(
    t,
    validationErrors?.['pacing.minIntervalMs']
  );
  const jitterMsError = getValidationMessage(
    t,
    validationErrors?.['pacing.jitterMs']
  );
  ...
  const sections = useMemo<VisualSection[]>(
    () => [
      ...
      {
        id: 'pacing' as VisualSectionId,
        title: t('config_management.visual.sections.pacing.title'),
        description: t('config_management.visual.sections.pacing.description'),
        icon: IconSlidersHorizontal,
        errorCount: countErrors(['pacing.maxConcurrency', 'pacing.minIntervalMs', 'pacing.jitterMs']),
      },
    ],
      ...
      <div ref={workspaceRef} className={styles.workspace}>
        ...
        <div className={styles.sections}>
          ...
          <ConfigSection
            id="pacing"
            ref={(node) => {
              sectionRefs.current.pacing = node;
            }}
            indexLabel="10"
          ...
          >
            <SectionStack>
              <ToggleRow
                title={t('config_management.visual.sections.pacing.enable')}
                description={t('config_management.visual.sections.pacing.enable_desc')}
                checked={values.pacing.enable}
                disabled={disabled}
                onChange={(enable) =>
                  onChange({
                    pacing: {
              ...
              {values.pacing.enable ? (
                <>
                  <Divider />
                  <SectionGrid>
                    <FieldShell
                      label={t('config_management.visual.sections.pacing.max_concurrency')}
                      htmlFor={maxConcurrencyInputId}
                      hint={t('config_management.visual.sections.pacing.max_concurrency_hint')}
                      hintId={maxConcurrencyHintId}
                      error={maxConcurrencyError}
                      errorId={maxConcurrencyErrorId}
                    >
                      <input
                        ...
                        type="number"
                        placeholder="1"
                        value={values.pacing.maxConcurrency}
                        onChange={(e) =>
                          onChange({
                            pacing: {
                              ...values.pacing,
                    ...
                    <FieldShell
                      label={t('config_management.visual.sections.pacing.min_interval_ms')}
                      htmlFor={minIntervalMsInputId}
                      hint={t('config_management.visual.sections.pacing.min_interval_ms_hint')}
                      hintId={minIntervalMsHintId}
                      error={minIntervalMsError}
                      errorId={minIntervalMsErrorId}
                    >
                      <input
                        id={minIntervalMsInputId}
                        className="input"
                        type="number"
                        placeholder="2000"
                        value={values.pacing.minIntervalMs}
                        onChange={(e) =>
                          onChange({
                            pacing: {
                              ...values.pacing,
                              minIntervalMs: e.target.value,
                            },
                          })
                        }
                        disabled={disabled}
                    ...
                    <FieldShell
                      label={t('config_management.visual.sections.pacing.jitter_ms')}
                      htmlFor={jitterMsInputId}
                      hint={t('config_management.visual.sections.pacing.jitter_ms_hint')}
                      hintId={jitterMsHintId}
                      error={jitterMsError}
                      errorId={jitterMsErrorId}
                    >
                      <input
                        id={jitterMsInputId}
                        className="input"
